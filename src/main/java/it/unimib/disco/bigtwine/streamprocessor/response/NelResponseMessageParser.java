...
public class NelResponseMessageParser extends AbstractResponseMessageParser<NelResponseMessage, LinkedTweetDTO>
    implements FlatMapFunction<String, LinkedTweetDTO> {
    ...
    @Override
    public void flatMap(String value, Collector<LinkedTweetDTO> out) throws Exception {
        NelResponseMessage res = this.parse(value);

        for (LinkedTweetDTO tweet : res.getTweets())  {
            out.collect(tweet);
        }

        LOG.debug("Finished nel processing, {} tweets collected", res.getTweets().length);
    }
}
